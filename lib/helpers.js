...
export async function goToContentBlockPage(page, contentBlockPath, blockTitle) {
  ...
  await page.getByRole("searchbox", { name: "Keyword" }).fill(blockTitle);
...
export async function embedInWhitehallDoc(
  ...
  await page.getByRole('radio', { name: 'News story', exact: true }).click();
  await page.getByRole("button", { name: "Next" }).click();
  ...
  // Reload page after setting tags - for some reason they sometimes don't get registered the first time
  await page.waitForLoadState();
  await page.reload();
...
export async function embedInMainstreamDoc(
  ...
  await page.getByRole("button", { name: "Save" }).click();
  await page.getByRole("link", { name: "Send to 2i" }).click();
  await page.getByRole("button", { name: "Send to 2i" }).click();
  await page.getByRole("link", { name: "Skip review" }).click();
  await page.getByRole("button", { name: "Skip review" }).click();
  await page.getByRole("button", { name: "Publish" }).click();
  await page.locator("button").filter({ hasText: "Send to publish" }).click();

  const url = await waitForUrlToBeAvailable(
    page,
    await page
      .getByRole("link", { name: "View on GOV.UK" })
      .getAttribute("href"),
  );
